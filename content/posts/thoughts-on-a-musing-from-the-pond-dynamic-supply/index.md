---
title: "Thoughts on \"a\" musing on the pond: Dynamic supply"
...
_Search "A Holistic Proposal for Change" in the article. John: sorry I cut up your holistic proposal (this is only a part of it.)_
...
The Algorand protocol, like most other blockchain protocols, can be changed - through consensus. This section briefly (and possibly inaccurately) describes the process.
...
- Develop new code, publish on Github, cut and test a release.
  - This is for small (parameter) protocol changes as well.
- An initial deployment is done to `betanet`.
  - This network's raison d'etre is to test new protocol versions without impacting `testnet`, which should be a stable network for developers to test on.
...
This was based on an honest response by Fabrice in the Algorand forums early on (2021?), which explained that: Algorand is upgradable software, so yes, technically it is possible to change that parameter by changing how the protocol works.
...
So if "today" played out under the new protocol, it would result **deflation** of 0.2529 ALGO per block.
...
We had 30,103 blocks produced today, so the day's deflation would be 7613.0487 and the yearly deflation (in this ground-hog day/year at the pond) would be 2,778,762.78 $ALGO.

I wouldn't say no to getting a tip like that in my wallet, but what does it look like in context? 

The total supply of 10,000,000,000.00 ALGO would become 9,997,221,237.22 ALGO.
...
### OK alright shut up but what if we have a very bad TPS year
...
Make a copy of it (or download it) to be able to tweak the inputs.

## Conclusion

While the "fixed supply" model gave the community a sense of security and comfort that "beyond 2030 there would be no inflation", changing this assumption and model is absolutely worth considering. The upside to a change like this is moving the protocol towards self-sustainability, and incentivising block proposers.

The downside to this particular instance of the model (with the specific parameters) is almost too small to measure. The 5 TPS scenario above would come with much larger problems than 0.2% inflation over a decade: it would mean that the blockchain has failed to get traction.
